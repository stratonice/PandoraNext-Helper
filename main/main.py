...
def refresh(user_id):
    ...
    access_token, session_token, my_sharetoken, pool_token = None, None, None, None
    ...
    if user.refresh_token is not None:
        ...
        # 使用更新后的 access token 获取 my_sharetoken
        try:
            my_sharetoken = share_tools.get_share_token(access_token, 'myshare')
            db.session.query(User).filter_by(id=user_id).update(
                {'my_sharetoken': my_sharetoken['token_key'], 'update_time': datetime.now()})
            db.session.commit()
        except Exception as e:
            logger.error(e)
            raise e

        # 使用 my_sharetoken 继续获取 pool_token
        try:
            pool_token = pool_tools.get_pool_token(my_sharetoken)
            db.session.query(User).filter_by(id=user_id).update(
                {'pool_token': pool_token, 'update_time': datetime.now()})
            db.session.commit()
        except Exception as e:
            logger.error(e)
            raise e
